...
CREATE TABLE IF NOT EXISTS tbl_K8SResourceIPBind (
    k8sresource_id VARCHAR(192) NOT NULL,          -- deployment：k8sclusterid-namespace-resource_name statefulset：k8sclusterid-namespace-pod_name
...
CREATE TABLE IF NOT EXISTS tbl_K8SScaleDownMark (
    k8sresource_id VARCHAR(192) NOT NULL,          -- k8sclusterid-namespace-resource_name 这里不区分deployment和statefulset
    k8sresource_type INT NOT NULL,                 -- 资源类型，Job CronJob proto.K8SApiResourceKindType
	current_replicas INT NOT NULL,		           -- scaledown后的pod数量
    scaledown_count INT NOT NULL,                  -- 要释放的pod数量
